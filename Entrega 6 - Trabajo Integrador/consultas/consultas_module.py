...
def filter_provinces(population_num, greater_less):
    ...
    with custom_poblacion.open(
        ...
        if greater_less == "mayor":
            provinces_meet = list(
                filter(lambda x: int((x[1])) > int(population_num), population_reader)
            )
        else:
            provinces_meet = list(
                filter(lambda x: int((x[1])) < int(population_num), population_reader)
        ...
        with custom_conectividad.open(mode="r", encoding="utf-8") as read_conect:
            ...
            for row in connectivity:
                ...
                if prov_connect in provinces_meet:
                    for i in range(4, 13):
                        if row[i] == "SI":
                            province_data[prov_connect]["conectividad"][
                                header[i]
                            ] = "SI"
        return province_data
